...
class Action {
  ...
  */
  ...
  /**
  * Listens for actions based on a shorthand name, an `action` Object and
  *   a callback `handler`
  *
  ...
  * @return {Action}
  */
  listen(shortName, action, handler) {
  ...
  load(pathname) {
    ...
    fs.readdirSync(pathname)
      ...
      .forEach(name => {
        ...
        this.listen(name, action, handler);
      });
    return this;
  }
  ...
  * @return {undefined}
  */
  test(shortName, query, callback) {
    let body = new ActionTest(this._name, this.formatIntentName(shortName), query).toObject();
    return this.execute(body, callback);
  }
